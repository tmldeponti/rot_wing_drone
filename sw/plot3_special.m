...
for j=1:19
    ...
    switch i
        case {1,2,3,4,5}
            ...
            Z_i = Z_i+p(end);
        case {6,7,8}
            p = lsqr([sin(deg2rad(x)).^(i-5).*y.^(i-5)],z,1e-12,50);
            z_i = p(1).*sin(deg2rad(x)).^(i-5).*y.^(i-5);
            Z_i = p(1).*sin(deg2rad(X)).^(i-5).*Y.^(i-5);
        case {9,10,11}
            p = lsqr([sin(deg2rad(x)).^(i-8),y.^(i-8)],z,1e-12,50);
            z_i = p(1).*sin(deg2rad(x)).^(i-8)+p(2).*y.^(i-8);
            Z_i = p(1).*sin(deg2rad(X)).^(i-8)+p(2).*Y.^(i-8);
        case {12,13,14}
            p = lsqr([sin(deg2rad(x)).^(i-11).*y.^(i-11),ones(size(x))],z,1e-12,50);
            z_i = p(1).*sin(deg2rad(x)).^(i-11).*y.^(i-11)+p(2);
            Z_i = p(1).*sin(deg2rad(X)).^(i-11).*Y.^(i-11)+p(2);
        case {15,16,17}
            p = lsqr([sin(deg2rad(x)).^(i-14),y.^(i-14),ones(size(x))],z,1e-12,50);
            z_i = p(1).*sin(deg2rad(x)).^(i-14)+p(2).*y.^(i-14)+p(3);
            Z_i = p(1).*sin(deg2rad(X)).^(i-14)+p(2).*Y.^(i-14)+p(3);
        case 18
            p = lsqr([sin(deg2rad(x)).^3.*y.^(2)],z,1e-12,50);
