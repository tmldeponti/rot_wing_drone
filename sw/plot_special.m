function [rmse_log,p_out] = plot_special(x,z,order,ii)
...
Z = zeros(size(x1));
RMSE = 10000;
case_list = ["poly 1","poly 2","poly sin(x)","poly (sin(x))^2","cos(x)-cos(x)^3","poly cos(x)","poly (cos(x))^2","sin(x)^3","sin(x)^2","x^2","sin(x)^2+k"];
best_case = 1;
if order < 0
    override = 0;
else
